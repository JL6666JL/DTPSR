...
(See paper for full details.) :contentReference[oaicite:1]{index=1}

---

## 🔥 Highlights

- **Disentangled textual priors**: global caption + per-segment LF/HF descriptions, enabling controllable, interpretable guidance. :contentReference[oaicite:2]{index=2}
- **Progressive semantic injection** via specialized cross-attention modules:
  ...
  - LRCA (LR Feature Cross-Attention for identity anchoring) :contentReference[oaicite:3]{index=3}
- **DisText-SR dataset** (~95k): global + region-wise LF/HF descriptions generated using panoptic segmentation + MLLM prompting. :contentReference[oaicite:4]{index=4}
...
3. Fuses LR visual features through **LRCA** to maintain image identity and consistency :contentReference[oaicite:5]{index=5}
...
Texts are encoded by a frozen CLIP text encoder to form embeddings for guidance. :contentReference[oaicite:6]{index=6}

> TODO: Provide dataset download link / license / checksum.

---
conda create -n dtpsr python=3.10 -y
conda activate dtpsr
